...
run_convex_backend() {
    ...
    local backend_bin="${script_dir}/convex-local-backend"
    ...
    if [ ! -f "$backend_bin" ]; then
        ...
        if command -v convex-local-backend &> /dev/null; then
            ...
            echo "ERROR: convex-local-backend binary not found at ${backend_bin} or in PATH"
            exit 1
        fi
    fi
    ...
    echo "=================================================="
    local admin_key
    admin_key=$(python "${script_dir}/generate_admin_key.py" "${instance_name}" "${hex_secret}")
